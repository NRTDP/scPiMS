...
```{r}
.packages = c("BiocManager","devtools","plyr","dplyr","readxl","tidyverse","tidyr", "doBy","GSVA","GSVAdata","plotly","msigdb","M3C", "GSEABase", "jsonify","factoextra","MASS","ggplot2","ComplexHeatmap","ggrepel")
...
data <- read_excel(paste0(data_dir,"pca_input_w_humanGeneID_5230&targeted.xlsx"), col_names = T,
                   sheet ="pca_input_w_humanGeneID_5230&ta" )|>as.data.frame()

rownames(data) <- data$CellNumber

pfr_anno <- read_excel(paste0(data_dir,"pca_input_w_humanGeneID_5230&targeted.xlsx"), sheet = "Sheet1")

cell_anno <- read_excel(paste0(data_dir,"pca_input_w_humanGeneID_5230&targeted.xlsx"), sheet = "Sheet2")
...
for (i in 1:length(Pathw_list)) {
  ...
  genes <- geneIds[geneIds %in% rownames(scores_data2)]
...
ComplexHeatmap::Heatmap(gsva.es2, top_annotation = column_ha,
                        show_row_names = T,
                        ...
                        row_title = "Pathways",
                        name = "score",
                        column_split = paste0("", pam$clustering))
```
...
protein_columns = 6:ncol(gsva.es3)
...
tsne <- Rtsne(train[,6:ncol(train)], dims = 2, perplexity=45, 
              check_duplicates = F,
...
umap_df <- gsva.es3[,6:ncol(gsva.es3)]
umap_meta <- gsva.es3[,1:5]
